% MATLAB Code for Creating Communication Signal Dataset - NO PADDING VERSION
...
fprintf('Dataset Configuration (Paper-Compliant with NO PADDING):\n');
...
for snr_idx = 1:length(SNR)
    ...
    for class_num = 1:num_classes
        ...
        for sig = 1:class_samples
            ...
            % Generate random data bits - ADD 10 EXTRA BITS
            data_bits = randi([0, 1], input_bits + 10, 1);  % Modified: +10 extra bits
            
            % Apply encoding
            encoded_bits = apply_encoding(data_bits, encoder_type);
            
            % Apply modulation - NO PADDING VERSION
            modulated_signal = apply_modulation_no_padding(encoded_bits, modulator_type);
            
            % Ensure signal length matches target - TRUNCATE AT STORAGE
            if length(modulated_signal) > signal_length
                modulated_signal = modulated_signal(1:signal_length);  % Truncate
            elseif length(modulated_signal) < signal_length
                ...
                modulated_signal = modulated_signal(1:signal_length);  % Final truncation
            end
...
summary_info.dataset_source = 'Paper-compliant with NO PADDING and 10 extra input bits';
summary_info.files_created = {};

for i = 1:length(SNR)
    filename = sprintf('paper_dataset_no_padding_SNR_%d_dB.mat', SNR(i));
    summary_info.files_created{i} = filename;
end

save('paper_dataset_no_padding_summary.mat', 'summary_info', '-v7.3');

fprintf('\n=== Paper Dataset Generation Complete (NO PADDING VERSION)! ===\n');
...
fprintf('Extra input bits: 10 per signal\n');
fprintf('Padding: NONE (natural signal characteristics preserved)\n');
...
fprintf(' - paper_dataset_no_padding_summary.mat (summary information)\n');
...
function encoded_bits = apply_encoding(data_bits, encoder_type)
    switch encoder_type
        ...
        case 'turbo_31'
            ...
            feedback = 31; 
            generators = [15 13];  % Octal polynomials
            ...
            % Encoder 1: original sequence
            encoded1 = convenc(u, trellis);
            encoded1 = reshape(encoded1, [], length(generators));
            parity1 = encoded1(:,2);   % take second output as parity
            
            % Interleaver: pseudorandom permutation
            rng(42);  % fix seed for reproducibility
            interleaver_idx = randperm(K).';
            u_int = u(interleaver_idx);
            
            % Encoder 2: interleaved sequence
            encoded2 = convenc(u_int, trellis);
            encoded2 = reshape(encoded2, [], length(generators));
            parity2 = encoded2(:,2);   % take second output as parity
            
            % Collect outputs: [systematic, parity1, parity2]
...
% MODIFIED MODULATION FUNCTION - NO PADDING VERSION
function modulated_signal = apply_modulation_no_padding(bits, modulator_type)
    switch modulator_type
        case '8fsk'  % NO PADDING - Process complete symbols only
            bits_per_symbol = 3;
            % Process only complete symbols
            num_complete_symbols = floor(length(bits) / bits_per_symbol);
            if num_complete_symbols > 0
                bits_to_use = bits(1:num_complete_symbols * bits_per_symbol);
                symbols = bi2de(reshape(bits_to_use, bits_per_symbol, [])', 'left-msb');
            else
                symbols = []; % Handle case with insufficient bits
            end
            
            try
                if ~isempty(symbols)
                    modulated_signal = fskmod(symbols, 8, 8, 2); % nsamp = 2
                    if size(modulated_signal, 2) > 1
                        modulated_signal = modulated_signal(:, 1); % Take first sample per symbol
                    end
                else
                    modulated_signal = complex(zeros(1,1));
                end
            catch
                % Manual FSK implementation
                if ~isempty(symbols)
