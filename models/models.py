...
def RF_MTL_CNN_Transformer(
    input_shape=(2048, 2, 1),
    embed_dim=128,
    num_heads=4,
    ff_dim=256,
    ...
    inp = layers.Input(shape=input_shape, name="signal_input")

    x = layers.Conv2D(64, (7, 2), strides=(2, 1), padding='same', activation=layers.LeakyReLU())(inp)
    x = layers.BatchNormalization()(x)
    x = layers.Dropout(0.1)(x)

    x = layers.Conv2D(128, (5, 2), strides=(2, 1), padding='same', activation=layers.LeakyReLU())(x)
    x = layers.BatchNormalization()(x)

    x = layers.Conv2D(embed_dim, (3, 2), strides=(2, 1), padding='same', activation=layers.LeakyReLU())(x)
    x = layers.BatchNormalization()(x)

    x = layers.Conv2D(embed_dim, (1, 2), padding='valid', activation=layers.LeakyReLU())(x)
    x = layers.BatchNormalization()(x)

    seq_len = int(x.shape[1])
    d_model = int(x.shape[3])
    x = layers.Reshape((seq_len, d_model))(x)

    positions = tf.range(start=0, limit=seq_len, delta=1)
    pos = layers.Embedding(input_dim=seq_len, output_dim=d_model)(positions)
    pos = tf.expand_dims(pos, axis=0)
    x = layers.Add(name="add_pos")([x, pos])

    for _ in range(transformer_blocks):
        attn = layers.MultiHeadAttention(num_heads=num_heads, key_dim=max(1, d_model // num_heads))(x, x)
    ...
    h_mod = layers.Dense(128, activation='relu')(feat)
    h_mod = layers.Dropout(dropout)(h_mod)
    out_mod = layers.Dense(num_mod_classes, activation='softmax', name='mod_4')(h_mod)

    h_enc = layers.Dense(128, activation='relu')(feat)
    h_enc = layers.Dropout(dropout)(h_enc)
    out_enc = layers.Dense(num_enc_classes, activation='softmax', name='enc_4')(h_enc)

    outputs = [out_mod, out_enc]

    if use_joint and num_joint_classes:
        h_joint = layers.Dense(256, activation='relu')(feat)
        h_joint = layers.Dropout(0.35)(h_joint)
        out_joint = layers.Dense(num_joint_classes, activation='softmax', name='joint_16')(h_joint)
        outputs = [out_joint, out_mod, out_enc]

    return Model(inputs=inp, outputs=outputs, name="RF_MTL_CNN_Transformer")

def iq_residual_block(input_tensor, filters, kernel_height=7, dropout_rate=0.25):

    x = Conv2D(filters, (kernel_height, 2), padding='same')(input_tensor)
    x = BatchNormalization()(x)
    x = LeakyReLU()(x)
   
    x = Conv2D(filters, (kernel_height, 2), padding='same')(x)
...
def SignalIQ_CNN():
    ...
    inputs = Input(shape=(2040, 2, 1), name='signal_input')

    x = Conv2D(64, (7, 2), strides=(2, 1), padding='same')(inputs) 
    x = BatchNormalization()(x)
    x = LeakyReLU()(x)

    
    x = iq_residual_block(x, 64)
    x = iq_residual_block(x, 64)
    x = MaxPooling2D(pool_size=(2, 1), padding='same')(x) 

    
    x = iq_residual_block(x, 128)
    x = iq_residual_block(x, 128)
    x = MaxPooling2D(pool_size=(2, 1), padding='same')(x) 

    
    x = iq_residual_block(x, 256, kernel_height=5) 
    x = iq_residual_block(x, 256, kernel_height=5)
    x = MaxPooling2D(pool_size=(2, 1), padding='same')(x)
...
def signal_resnet_block(input_tensor, filters, kernel_size=(2, 5)):
...
def SignalResNet_Lite():
 
    inputs = layers.Input(shape=(2040, 2, 1), name='signal_input')

    x = layers.Conv2D(32, (2, 7), strides=(1, 2), padding='same')(inputs) # Filters reduced from 64 to 32
    x = layers.BatchNormalization()(x)
    x = layers.LeakyReLU()(x)

   
    x = signal_resnet_block(x, 32) 
    x = signal_resnet_block(x, 32)
    x = layers.MaxPooling2D(pool_size=(2, 1))(x)

    x = signal_resnet_block(x, 64) 
    x = signal_resnet_block(x, 64)
    x = layers.MaxPooling2D(pool_size=(2, 1))(x)

  
    x = signal_resnet_block(x, 128) 
    x = signal_resnet_block(x, 128)
    x = layers.MaxPooling2D(pool_size=(2, 1))(x)
...
def cnn_feature_extractor(inputs, embed_dim):
  
    x = Conv2D(64, (7, 2), strides=(2, 1), padding='same', activation=LeakyReLU())(inputs)
    x = BatchNormalization()(x)
    x = MaxPooling2D(pool_size=(2, 1), padding='same')(x)


    x = Conv2D(128, (5, 2), padding='same', activation=LeakyReLU())(x)
    x = BatchNormalization()(x)
    x = MaxPooling2D(pool_size=(2, 1), padding='same')(x)

    x = Conv2D(embed_dim, (3, 2), padding='valid', activation=LeakyReLU())(x)
    x = BatchNormalization()(x)
...
def SignalTransformer(
    input_shape=(2040, 2, 1),
    embed_dim=128,      
    num_heads=8,       
    ff_dim=256,         
    dropout_rate=0.2
    ...
    cnn_features = cnn_feature_extractor(inputs, embed_dim)
    reshaped = Reshape((cnn_features.shape[1], cnn_features.shape[3]))(cnn_features)
    
    positions = tf.range(start=0, limit=reshaped.shape[1], delta=1)
    pos_embedding = Embedding(input_dim=reshaped.shape[1], output_dim=embed_dim)(positions)
    x = reshaped + pos_embedding

    for _ in range(num_transformer_blocks):
        x = transformer_encoder_block(x, embed_dim // num_heads, num_heads, ff_dim, dropout_rate)
